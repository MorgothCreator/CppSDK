...
char *tftp_errorcode_string[] = {
                                  (char *)"not defined",
								  (char *)"file not found",
								  (char *)"access violation",
								  (char *)"disk full",
								  (char *)"illegal operation",
								  (char *)"unknown transfer id",
                          ...
                          const ip_addr_t *to_ip, u16_t to_port)
{
   ...
   * in "args->data" and return the number of bytes read */
   f_read(&file_SD, (unsigned char*)args->data, TFTP_DATA_LEN_MAX, (UINT*)(&args->data_len));
                       ...
                       const ip_addr_t *addr, u16_t port)
{
  ...
  if (tftp_is_correct_ack((char *)p->payload, args->block))
  {
    /* increment block # */
    args->block++;
  }
...
void wrq_recv_callback(void *arg, struct udp_pcb *upcb, struct pbuf *pkt_buf, const ip_addr_t *addr, u16_t port)
{
  ...
  if ((pkt_buf->len > TFTP_DATA_PKT_HDR_LEN) &&
      (tftp_extract_block((char *)pkt_buf->payload) == (args->block + 1)))
  {
...
int tftp_process_read(struct udp_pcb *upcb, const ip_addr_t *to, unsigned short to_port, char* FileName)
{
  ...
  char *path = (char *)malloc(strlen(FileName) + sizeof(TFTPD_PATH_SERVER_DRIVE_PATH) + 1);
  if(!path)
	  return 0;
  strcpy(path, TFTPD_PATH_SERVER_DRIVE_PATH);
...
void process_tftp_request(struct pbuf *pkt_buf, const ip_addr_t *addr, u16_t port)
{
  tftp_opcode op = tftp_decode_op((char *)pkt_buf->payload);
  char FileName[30];
  struct udp_pcb *upcb;
  err_t err;
  ...
  switch (op)
  {
    case TFTP_RRQ:/* TFTP RRQ (read request) */
    {
      ...
      if (f_opendir(&dir_1, TFTPD_PATH_SERVER_DRIVE_PATH"/") != FR_OK)
      {
        return;
      }
    ...
    case TFTP_WRQ: /* TFTP WRQ (write request) */
    {
      ...
      if (f_opendir(&dir_2, TFTPD_PATH_SERVER_DRIVE_PATH"/") != FR_OK)
      {
        return;
      }
