...
//###########################################################################
unsigned int fifo_push_buff(fifo_settings_t* param, unsigned char *buff,
		unsigned int len)
{
	unsigned int cnt = 0;
	for (; cnt < len; cnt++)
...
//###########################################################################
unsigned int fifo_pop_buff(fifo_settings_t* param, unsigned char *buff,
		unsigned int len)
{
	unsigned int cnt = 0;
	for (; cnt < len; cnt++)
...
{
	fifo_settings_t* Settings = (fifo_settings_t*) calloc(1,
			sizeof(fifo_settings_t));
	if (Settings == 0)
		return NULL ;
	char* Buff;
