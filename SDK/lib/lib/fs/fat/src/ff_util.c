...
/*-----------------------------------------------------------------------*/
FRESULT ff_util_write(FIL* fp, /* Pointer to the file object */
...
/*-----------------------------------------------------------------------*/
int ff_util_gets(TCHAR* buff, /* Pointer to the string buffer to read */
...
/*-----------------------------------------------------------------------*/
int ff_util_appendc(TCHAR c, /* A character to be output */
FIL* fp /* Pointer to the file object */
)
{
	checksum_send -= c;
...
/*-----------------------------------------------------------------------*/
int ff_util_putc(TCHAR c, /* A character to be output */
FIL* fp /* Pointer to the file object */
)
{
	checksum_send -= c;
...
/*-----------------------------------------------------------------------*/
int ff_util_appends(const TCHAR* str, /* Pointer to the string to be output */
FIL* fp /* Pointer to the file object */
)
{
	if (f_size(fp) != 0)
...
/* Put a string to file                                              */
/*-----------------------------------------------------------------------*/
unsigned int ff_util_puts(const TCHAR* str, /* Pointer to the string to be output */
FIL* fp /* Pointer to the file object */
)
{
	unsigned int n;
...
/*-----------------------------------------------------------------------*/
int ff_util_appendhexs(const unsigned char* buff, /* Pointer to the buffer to be output */
...
/* Put a next line                                                       */
/*-----------------------------------------------------------------------*/
int ff_util_put_nextline(FIL* fp /* Pointer to the file object */
)
{
	char str[3];
	str[0] = '\r';
...
int ff_vsnprintf_(FIL* fp, char *str, size_t count, const char *fmt,
		va_list arg); // Renamed to avoid conflict with builtin 'vsnprintf' function.
...
static void dopr_outch(FIL* fp, char *buffer, size_t *currlen, size_t maxlen,
		char c);
	...
	while (state != DP_S_DONE)
	{
		...
		case DP_S_CONV:
			...
			case 'i':
				...
				fmtint(fp, buffer, &currlen, maxlen, value, 10, min, max,
						flags);
				break;
			case 'o':
				flags |= DP_F_UNSIGNED;
			...
			case 'x':
				...
				if (cflags == DP_C_SHORT)
					...
					value = va_arg(args, unsigned int);
				fmtint(fp, buffer, &currlen, maxlen, value, 16, min, max,
						flags);
				break;
			case 'f':
			...
			case 'p':
				strvalue = va_arg(args, char *);
				fmtint(fp, buffer, &currlen, maxlen, (int) strvalue, 16, min,
						max, flags);
				break;
			case 'n':
				if (cflags == DP_C_SHORT)
 ...
 *
 */
int ff_util_printf(FIL* fp, /* Pointer to the file object */
...
#endif
{
	...
	VA_START(str);VA_SHIFT(str, char *);VA_SHIFT(count, size_t);VA_SHIFT(fmt,
			char *);
	char buff[1];
...
/* Seek                                                       */
/*-----------------------------------------------------------------------*/
FRESULT ff_util_seek(FIL* fp, /* Pointer to the file object */
...
/* Seek to end of file                                                       */
/*-----------------------------------------------------------------------*/
FRESULT ff_util_seek_eof(FIL* fp /* Pointer to the file object */
)
{
...
/* Check if file exist                                                   */
/*-----------------------------------------------------------------------*/
FRESULT ff_util_file_exist(const TCHAR* str /* Pointer to the string to be output */
)
{
	FIL fp; /* Pointer to the file object */
