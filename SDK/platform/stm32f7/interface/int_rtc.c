...
bool _rtc_set_calendar(rtc_t *calendar, unsigned char HourFormat)
{
	...
	sdatestructure.WeekDay = rtc_weekday_convert(calendar->Year,
...
bool _rtc_set_register_8bit(unsigned int addr, unsigned char value)
{
	if (!rtc_init_status)
		return rtc_init_status;
	HAL_RTCEx_BKUPWrite(&RtcHandle, addr >> 2,
			HAL_RTCEx_BKUPRead(&RtcHandle,
					(addr >> 2) & (0xFF << ((addr & 0x03) << 3)))
					+ (value << ((addr & 0x03) << 3)));
	return true;
}
...
bool _rtc_set_register_16bit(unsigned int addr, unsigned short value)
{
	if (!rtc_init_status)
		return rtc_init_status;
	HAL_RTCEx_BKUPWrite(&RtcHandle, addr >> 1,
			HAL_RTCEx_BKUPRead(&RtcHandle,
					(addr >> 1) & (0xFFFF << ((addr & 0x01) << 4)))
					+ (value << ((addr & 0x01) << 4)));
	return true;
}
...
bool _rtc_setup_tamper()
{
	...
	stamperstructure.TimeStampOnTamperDetection =
