...
#include <interface/qspi.h>
#include <interface/qspi_def.h>
...
static uint8_t QSPI_N24Q_AutoPollingMemReady(Qspi_Mem_t *param,
		uint32_t Timeout);
 ...
 * @retval QSPI memory status
 */
uint8_t QSPI_N24Q_Read(Qspi_Mem_t *param, uint8_t* pData, uint32_t ReadAddr,
		uint32_t Size)
{
 ...
 * @retval QSPI memory status
 */
uint8_t QSPI_N24Q_Write(Qspi_Mem_t *param, uint8_t* pData, uint32_t WriteAddr,
		uint32_t Size)
{
	QSPI_CommandTypeDef s_command;
	...
	do
	{
		...
		if (QSPI_N24Q_AutoPollingMemReady(param,
		...
		pData += current_size;
		current_size =
				((current_addr + param->dev_info->PageSize) > end_addr) ?
						(end_addr - current_addr) : param->dev_info->PageSize;
...
uint8_t QSPI_N24Q_Erase_Block(Qspi_Mem_t *param, uint32_t BlockAddress)
{
	...
	if (QSPI_N24Q_AutoPollingMemReady(param,
			param->dev_info->SubSectorEraseMaxTime) != QSPI_OK)
	{
		return QSPI_ERROR;
	}
...
uint8_t QSPI_N24Q_Erase_Chip(Qspi_Mem_t *param)
{
	...
	/* Configure automatic polling mode to wait for end of erase */
	if (QSPI_N24Q_AutoPollingMemReady(param,
			param->dev_info->BulkEraseMaxTime) != QSPI_OK)
	{
		return QSPI_ERROR;
	}
...
static uint8_t QSPI_N24Q_DummyCyclesCfg(Qspi_Mem_t *param)
{
	...
	MODIFY_REG(reg, VCR_NB_DUMMY,
			(param->dev_info->DumyCiclesReadQuad << POSITION_VAL(VCR_NB_DUMMY)));
	...
	/* Transmission of the data */
	if (HAL_QSPI_Transmit(&QSPIHandle, &reg, HAL_QPSI_TIMEOUT_DEFAULT_VALUE)
			!= HAL_OK)
	{
		return QSPI_ERROR;
	}
