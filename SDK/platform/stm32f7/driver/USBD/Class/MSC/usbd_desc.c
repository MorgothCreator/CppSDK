...
#pragma data_alignment=4   
#endif
__ALIGN_BEGIN uint8_t USBD_StrDescMsc[USBD_MAX_STR_DESC_SIZ] __ALIGN_END;
		...
		uint16_t *length)
{
	...
	{
		USBD_GetString((uint8_t *) USBD_PRODUCT_HS_STRING, USBD_StrDescMsc,
				length);
	}
	else
	{
		USBD_GetString((uint8_t *) USBD_PRODUCT_FS_STRING, USBD_StrDescMsc,
				length);
	}
	return USBD_StrDescMsc;
}
...
uint8_t *USBD_MSC_ManufacturerStrDescriptor(USBD_SpeedTypeDef speed,
		uint16_t *length)
{
	USBD_GetString((uint8_t *) USBD_MANUFACTURER_STRING, USBD_StrDescMsc,
			length);
	return USBD_StrDescMsc;
}
...
uint8_t *USBD_MSC_ConfigStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
	if (speed == USBD_SPEED_HIGH)
	{
		USBD_GetString((uint8_t *) USBD_CONFIGURATION_HS_STRING,
				USBD_StrDescMsc, length);
	}
	else
	{
		...
		uint16_t *length)
{
	if (speed == USBD_SPEED_HIGH)
	{
		USBD_GetString((uint8_t *) USBD_INTERFACE_HS_STRING, USBD_StrDescMsc,
				length);
	}
	else
	{
		USBD_GetString((uint8_t *) USBD_INTERFACE_FS_STRING, USBD_StrDescMsc,
				length);
	}
	return USBD_StrDescMsc;
}
